...
double divideAndConquer(vector<coordinate_t> od_by_x, vector<coordinate_t> od_by_y, int start, int end) {
    ...
    double left_d_bound = od_by_x[index].point.first - min_dist, right_d_bound = od_by_x[index].point.first + min_dist;
    for(i = 0 ; i < od_by_y.size() ; i++) {
        if(left_d_bound <= od_by_y[i].point.first &&  od_by_y[i].point.first <= right_d_bound) {
    ...
    for (i=0; i<tmp.size()-1; ++i){
		for (int j=1; j<=3 && i+j<tmp.size(); ++j){
            x_coor_1 = tmp[i].point.first;
            y_coor_1 = tmp[i].point.second;
            x_coor_2 = tmp[i+j].point.first;
            y_coor_2 = tmp[i+j].point.second;
            tmp_dist = sqrt(pow(x_coor_1 - x_coor_2, 2) + pow(y_coor_1 - y_coor_2, 2));
...
int main() {
    ...
    while (case_n--) {
        ...
        for (i = 0; i < dist_n; i++) {
            scanf("%lf%lf", &points[i].point.first, &points[i].point.second);
            points[i].id = i;
        }
